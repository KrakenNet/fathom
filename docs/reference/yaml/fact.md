...
The endpoint (`src/fathom/integrations/rest.py` lines 547–564) requires
...
Single (`src/fathom/engine.py` line 1309):

```python
engine.assert_fact("access-request", {"subject": "alice", "action": "read"})
```

Atomic batch (`src/fathom/engine.py` line 1365) — every fact is
