...
- **Loading the same directory twice is a no-op.** Identity is the resolved
  path, so a relative and an absolute spelling are one pack, not two.
- **A second pack may not redefine a template the first registered.** The
  ...
  untouched. This is the same check `load_pack` applies.
- **A directory holding nothing it recognises is an error**, not an engine
  with no rules in it. Pointing one level too high used to succeed silently.
